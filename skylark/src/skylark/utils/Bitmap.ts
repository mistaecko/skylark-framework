...
module skylark {
    ...
    var _scaleRows = function (rows, scale:number) {
        // Simplest scaling possible
        var real_w:number = rows.length,
            scaled_w:number = parseInt(String(real_w * scale)),
            real_h:number = real_w ? rows[0].length : 0,
            scaled_h:number = parseInt(String(real_h * scale)),
            new_rows:any = [],
            new_row:any, x, y;
    ...
    export class Bitmap {
        ...
        public static create(rows, scale?:number):any {
            ...
            var height = rows.length,                                // the number of rows
                ...
                num_data_bytes:any = (width * 3 + row_padding) * height, // size in bytes of BMP data
